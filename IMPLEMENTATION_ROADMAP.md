...
**Follow-up — public site rebuilt (`index.html` rewritten, `about.html` added).** The open question of whether the static prototype was a design reference or dead weight is resolved: it is now the app's real public landing page.

- **The page was rendering completely unstyled and nobody had noticed.** `index.html` sits at the application root but loaded `../assets/css/style.css` and `../assets/js/main.js` — one level *above* the root, so both 404'd. Fixed to `assets/…`, which is what actually made the design appear.
- **Every broken link fixed.** `login.html` (×9) and `../login.aspx` → `login.aspx`; `donate-form.html` (×6) → `Donor/donate-form.aspx`, which redirects a signed-out visitor to login and takes a signed-in donor straight to the form. `donations-list.html` (×3) was **removed rather than repointed** — no public donation list exists, and building one would publish donor addresses and contact details to anonymous visitors.
- **Fabricated content removed**, the same pass every dashboard got: the hero's "12,400+ meals / 340+ donors / 85 NGO partners", the four-figure stats strip, the mock dashboard's invented metrics and donor names, and the entire "Recent Donations" table (four invented donations naming real charities, dated April 2025). A static HTML file cannot query the database, so these could not be made real — they are replaced with statements that are true regardless of usage. The stats strip now carries four **checkable properties of the build**: 4 roles, 8 notification events, 5 fraud rules, 0 paid API keys.
- **Two features were being advertised that the app deliberately does not have**: "Digital appreciation certificate" and "In-system messaging with donors" — the same two whose sidebar links were deleted a few days earlier for not existing. Both are gone from the marketing copy.
- **A real features section** now lists all twelve shipped capabilities (donation posting, admin approval, NGO claiming, volunteer assignment, maps and live tracking, donation tracking, notifications, ratings and trust, emergency mode, fraud detection, reports and analytics, role-scoped search), each described in terms of what the code actually does, under a heading stating that nothing listed is planned or coming soon.
- **`about.html` is new**: the problem being addressed, the four principles the platform is built around, a technology section (`#tech`), and a contact section (`#contact`) using the project address already configured in `Web.config`. It states plainly that this is a final-year project with no support desk, and that registration does not grant immediate access.
- **Verification**: every link on both pages was resolved over HTTP from the rendered page, with cross-page fragments checked by fetching the target and confirming the `id` exists — **index.html 27 links, about.html 23 links, zero broken, zero missing anchors**, and `assets/css/style.css` and `assets/js/main.js` both 200. Footer social links and the Privacy Policy link were dropped (no accounts, no document).
